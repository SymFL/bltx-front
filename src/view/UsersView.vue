<template>
    ...
    <!-- 添加/编辑用户弹窗 -->
    <el-dialog v-model="dialogVisible" :title="this.userQuery.id > 0 ? '修改用户' : '添加用户'" width="750" center :before-close="handleClose" destroy-on-close>
        ...
        <el-form ref="userRefForm" :model="userQuery" label-width="140px" :rules="userRules">
            ...
            <el-form-item label="账号是否过期：" prop="accountNoExpired">
                <el-select v-model="userQuery.accountNoExpired" style="width: 240px">
                    <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
            <el-form-item label="密码是否过期：" prop="credentialsNoExpired">
                <el-select v-model="userQuery.credentialsNoExpired" style="width: 240px">
                    <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
            <el-form-item label="账号是否被锁定：" prop="accountNoLocked">
                <el-select v-model="userQuery.accountNoLocked" style="width: 240px">
                    <el-option v-for="item in options" :key="item.value" :label="item.label" :value="item.value" />
                </el-select>
            </el-form-item>
